...
unique(penguins_filtered$sex) 
min(penguins_filtered$culmen_length_mm)
```
...
mutate <- penguins |> 
  mutate(flippometers = flipper_length_mm / 1000000) # I could also use 1e6
...
final_boss <- penguins |> 
  ...
  summarize(avg_mass_g = round(mean(body_mass_g, na.rm = TRUE)))
